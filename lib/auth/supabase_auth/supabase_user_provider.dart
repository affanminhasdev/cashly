...
class CashlyCRMSupabaseUser extends BaseAuthUser {
  CashlyCRMSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> cashlyCRMSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CashlyCRMSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
