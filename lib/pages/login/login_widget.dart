    ...
    return Shortcuts(
      shortcuts: {
        SingleActivator(
          ...
          _model.userCopy = await UsuariosTable().queryRows(
            queryFn: (q) => q.eqOrNull(
              'uuid',
              currentUserUid,
            ),
          );
          if (_model.userCopy?.firstOrNull?.role == 1) {
            GoRouter.of(context).prepareAuthEvent();
            ...
            if (_model.userCopy!.firstOrNull!.status) {
              FFAppState().Usuario = UsuarioStruct(
                nome: _model.userCopy?.firstOrNull?.nome,
                email: _model.userCopy?.firstOrNull?.email,
                role: _model.userCopy?.firstOrNull?.role,
                cor: _model.userCopy?.firstOrNull?.cor,
              );

              context.goNamedAuth(LeadsWidget.routeName, context.mounted);
            } else {
              GoRouter.of(context).prepareAuthEvent();
              await authManager.signOut();
              GoRouter.of(context).clearRedirectLocation();
